    ...
    return Theme(
      ...
      child: Scaffold(
          ...
          body: GridView.builder(
            padding: EdgeInsets.only(left: 16, right: 16, top: MediaQuery.of(context).padding.top + AppBar().preferredSize.height),
...
class _SingleImageEditorState extends State<SingleImageEditor> {
  ...
  List<BottomButtonType> bottomButtonList = [
    BottomButtonType.crop,
    BottomButtonType.brush,
    BottomButtonType.text,
    BottomButtonType.flip,
    BottomButtonType.rotateLeft,
    BottomButtonType.rotateRight,
    BottomButtonType.blur,
    BottomButtonType.filter,
    BottomButtonType.emoji,
  ];
      ...
      child: Scaffold(
        ...
        bottomNavigationBar: Container(
          ...
          child: SafeArea(
            child: ListView.separated(
                ...
                itemBuilder: (context, index) {
                  return SizedBox(
                    width: 65,
                    child: BottomButton(
                      icon: bottomButtonList[index].icon,
                      text: bottomButtonList[index].value,
                      onTap: () {
                        onTap(bottomButtonList[index]);
                      },
                    ),
                  );
                },
            ...
            //       text: 'Blur',
            //       onTap: () {
            //         var blurLayerIndex = layers.indexWhere((element) => element is BackgroundBlurLayerData);
            //         BackgroundBlurLayerData blurLayer;
            //         if (blurLayerIndex < 0) {
            //           blurLayer = BackgroundBlurLayerData(
            //             color: Colors.transparent,
            //             radius: 0.0,
            //             opacity: 0.0,
            //           );
            //         } else {
            //           blurLayer = layers[blurLayerIndex] as BackgroundBlurLayerData;
            //         }
            //
            //         undoLayers.clear();
            //         removedLayers.clear();
  ...
  Future<void> loadImage(dynamic imageFile) async {
    ...
    originalBackgroundLayerData = BackgroundLayerData(
      file: currentImage,
    );

    layers.add(originalBackgroundLayerData!);

    setState(() {});
  }

  Future<void> onTap(BottomButtonType type) async {
    switch (type) {
      case BottomButtonType.crop:
        controllerCrop();
        break;
      case BottomButtonType.brush:
        controllerBrush();
        break;
      case BottomButtonType.text:
        controllerText();
        break;
      case BottomButtonType.flip:
        controllerFlip();
        break;
      case BottomButtonType.rotateLeft:
        controllerRotateLeft();
        break;
      case BottomButtonType.rotateRight:
        controllerRotateRight();
        break;
      case BottomButtonType.blur:
        controllerBlur();
        break;
      case BottomButtonType.filter:
        controllerFilter();
        break;
      case BottomButtonType.emoji:
        controllerEmoji();
        break;
    }
  }

  Future<void> controllerCrop() async {
    resetTransformation();
  ...
  Future<void> controllerFilter() async {
    ...
    //   }
    // }
    // var index = layers.indexWhere((element) => element is BackgroundLayerData);
    // var mergedImage = originalBackgroundLayerData?.file.image ?? layers/*await getMergedImage()*/;
    // var mergedImage = await getMergedImage();

    ImageFilterResult? result = await Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => ImageFilters(
          imageFilterResult: ImageFilterResult(image: originalBackgroundLayerData!.file.image, selectedFilter: selectedFilter),
        ),
      ),
    );
...
enum BottomButtonType { crop, brush, text, flip, rotateLeft, rotateRight, blur, filter, emoji }

extension BottomButtonTypeEx on BottomButtonType {
  IconData get icon {
    switch (this) {
      case BottomButtonType.crop:
        return Icons.crop;
...
class ImageFilterResult {
  ...
  ImageFilterResult({required this.image, this.selectedFilter});
}

/// Return filter applied Uint8List image
class ImageFilters extends StatefulWidget {
  // final Uint8List image;
  final ImageFilterResult imageFilterResult;
  ...
  const ImageFilters({
    Key? key,
    // required this.image,
    required this.imageFilterResult,
    this.useCache = true,
  }) : super(key: key);
...
class _ImageFiltersState extends State<ImageFilters> {
  ...
  @override
  void initState() {
    selectedFilter = widget.imageFilterResult.selectedFilter ?? PresetFilters.none;
          ...
          actions: [
            IconButton(
              ...
              onPressed: () async {
                ...
                var result = ImageFilterResult(image: data, selectedFilter: selectedFilter);
                Navigator.pop(context, result);
              },
            ).paddingSymmetric(horizontal: 8),
          ],
        ...
        body: Center(
          child: Screenshot(
            ...
            child: Stack(
              children: [
                Image.memory(
                  widget.imageFilterResult.image,
                  fit: BoxFit.cover,
                ),
                FilterAppliedImage(
                  image: widget.imageFilterResult.image,
